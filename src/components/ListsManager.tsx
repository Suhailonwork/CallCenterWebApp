...
export function ListsManager() {
  ...
  const [fields, setFields] = useState<string[]>([...DEFAULT_LIST_FIELDS]);
  ...
  async function remove(l: ListRow) {
    const msg =
      l.lead_count > 0
        ? `Delete list "${l.name}" and its ${l.lead_count} leads? Call history is kept. This cannot be undone.`
        : `Delete list "${l.name}"? This cannot be undone.`;
    ...
    if (res.ok) {
      toast.success(
        data.deletedLeads > 0
          ? `List deleted (${data.deletedLeads} leads removed)`
          : "List deleted",
      );
      load();
                ...
                <tr key={l.id} className="border-b border-slate-100 align-top">
                  ...
                  <td className="py-2">
                    <div className="flex flex-nowrap gap-1.5">
                      ...
                      <button
                        onClick={() => remove(l)}
                        title="Delete this list and its leads"

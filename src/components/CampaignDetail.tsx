...
/**
 * Full campaign detail (stats, agents, calls with recordings/notes/dispo).
 * Shared by the Admin and TL consoles — the API + navigation targets differ:
 *   Admin: apiBase="/api/admin", backHref="/admin/campaigns", customerBase="/admin/customers"
 *   TL:    apiBase="/api/tl",    backHref="/tl/campaigns",    customerBase="/tl/customers"
 */
export function CampaignDetail({
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await fetch(`${apiBase}/campaigns/${id}/details`, {
          cache: "no-store",
        });
        if (!res.ok) {
                ...
                <tr key={c.id} className="border-b border-slate-100">
                  <td className="px-3 py-2 font-medium">
                    <button
                      onClick={() =>
                        router.push(
                          `${customerBase}/${encodeURIComponent(c.phone_number)}`,
                        )
                      }

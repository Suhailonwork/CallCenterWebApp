...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  ...
  // Dial rules must leave the campaign able to dial SOMETHING — reject when
  // both dial_statuses and recycle_rules are explicitly emptied (the Dial
  // Rules modal always sends both, so this is the "nothing dials" case).
  if (
  ...
  try {
    ...
    await conn.commit();
    // Apply new dial rules to the running dialer immediately (config is read
    // fresh per claim; the kick just avoids waiting for the next timer tick).
...
/**
 * DELETE /api/admin/campaigns/:id — delete a campaign and everything under it.
 * FK cascades remove its lists + leads (fk_lists_campaign, fk_csv_campaign),
 * gateway/agent assignments (fk_cg_campaign, fk_ca_campaign); call history
 * survives (calls.campaign_id is ON DELETE SET NULL). Returns what was removed
 * so the UI confirmation can report it.
 */
export async function DELETE(_req: Request, { params }: { params: { id: string } }) {
  ...
  await logAudit({
    ...
    entity: 'campaigns',
    entityId: id,
    details: { name: info.name, deletedLists: Number(info.lists), deletedLeads: Number(info.leads) },
  });

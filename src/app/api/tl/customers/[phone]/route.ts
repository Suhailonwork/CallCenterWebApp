import { authenticate, isError, ok, fail } from '@/lib/api';
import { query, queryOne } from '@/lib/db';
import { groupIdsForTL } from '@/lib/groups';

export const runtime = 'nodejs';

/**
 * GET /api/tl/customers/[phone] — call / lead history for one phone number,
 * restricted to calls made in campaigns belonging to the TL's groups.
 * Mirrors the admin customers endpoint (notes, dispo, recordings).
 */
export async function GET(
  _req: Request,
  { params }: { params: { phone: string } },
) {
  const u = await authenticate(['tl']);
  if (isError(u)) return u;

  const phone = decodeURIComponent(params.phone).trim();
  if (!phone) return fail('Invalid phone number');

  const groupIds = await groupIdsForTL(u.id);
  if (groupIds.length === 0) return fail('No calls found for this number', 404);

  // RBAC: only calls whose campaign belongs to one of the TL's groups.
  const gph = groupIds.map(() => '?').join(',');
  const scope = `c.campaign_id IN (SELECT id FROM campaigns WHERE group_id IN (${gph}))`;

  const info = await queryOne<any>(
    `SELECT
          MAX(c.contact_name) AS contact_name,
          COUNT(*) AS total_calls,
          SUM(c.status IN ('connected','completed')) AS connected,
          SUM(c.status = 'completed') AS success,
          COALESCE(SUM(c.duration_seconds), 0) AS talk_seconds
     FROM calls c
    WHERE c.phone_number = ? AND ${scope}`,
    [phone, ...groupIds],
  );

  if (info) {
    info.phone_number = phone;
  }

  if (!info || Number(info.total_calls) === 0) {
    return fail('No calls found for this number', 404);
  }

  const byAgent = await query<any>(
    `SELECT u.id AS agent_id, u.name AS agent_name,
            COUNT(c.id) AS calls,
            SUM(c.status = 'completed') AS success,
            COALESCE(SUM(c.duration_seconds), 0) AS talk_seconds
       FROM calls c
       LEFT JOIN users u ON u.id = c.employee_id
      WHERE c.phone_number = ? AND ${scope}
      GROUP BY u.id, u.name
      ORDER BY calls DESC`,
    [phone, ...groupIds],
  );

  const calls = await query<any>(
    `SELECT c.id, c.campaign_id, c.direction, c.status,
            c.duration_seconds, c.recording_url,
            DATE_FORMAT(c.started_at, '%Y-%m-%d %H:%i:%s') AS started_at,
            u.name AS agent_name,
            cmp.name AS campaign_name,
            n.note AS note,
            n.tags AS dispo
       FROM calls c
       LEFT JOIN users u       ON u.id = c.employee_id
       LEFT JOIN campaigns cmp ON cmp.id = c.campaign_id
       LEFT JOIN call_notes n  ON n.call_id = c.id
      WHERE c.phone_number = ? AND ${scope}
      ORDER BY c.id DESC
      LIMIT 500`,
    [phone, ...groupIds],
  );

  return ok({ info, byAgent, calls });
}
